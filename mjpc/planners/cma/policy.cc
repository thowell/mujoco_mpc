...
// allocate memory
void CMAPolicy::Allocate(const mjModel* model, const Task& task,
                              int horizon) {
  // model
  this->model = model;
...
void CMAPolicy::Reset(int horizon) {
  // parameters
...
void CMAPolicy::Action(double* action, const double* state,
                            double time) const {
  // find times bounds
  int bounds[2];
...
// copy policy
void CMAPolicy::CopyFrom(const CMAPolicy& policy, int horizon) {
...
// copy parameters
void CMAPolicy::CopyParametersFrom(
