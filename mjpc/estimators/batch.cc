...
void Batch::Initialize(const mjModel* model) {
  ...
  // scratch
  scratch0_prior_.resize(std::max(3 * nv * 3 * nv, ntotal));
...
void Batch::Reset(const mjData* data) {
  ...
  // scale prior
  scale_prior = GetNumberOrDefault(0.0, model, "batch_scale_prior");
...
void Batch::Update(const double* ctrl, const double* sensor) {
  ...
  double* weights_band = weight_prior_band_.data();
...
double Batch::CostPrior(double* gradient, double* hessian) {
  ...
  for (int t = 0; t < configuration_length_; t++) {
    ...
    if (hessian) {
      ...
      // mat
      double* mat = scratch0_prior_.data();
  ...
  for (int t = 0; t < configuration_length_ - 1; t++) {
    ...
    for (int i = 0; i < nsensor_; i++) {
      ...
      if (hessian) {
        ...
        // add block in Hessian
        SetBlockInBand(hessian, tmp1, weight, nvar, nband, nband,
                       nv * std::max(0, t - 1));
      }
...
double Batch::CostForce(double* gradient, double* hessian) {
  ...
  for (int t = 1; t < configuration_length_ - 1; t++) {
    ...
    if (hessian) {
      ...
      // add block in Hessian
      SetBlockInBand(hessian, tmp1, 1.0, nvar, nband, nband, nv * (t - 1));
    }
  }
...
void Batch::TotalHessian(double* hessian) {
  ...
  // sum individual Hessians
  if (settings.prior_flag) {
    mju_copy(hessian, cost_hessian_prior_band_.data(), ntotal * nband);
  } else {
    // zero memory
...
void Batch::SearchDirection() {
  ...
  while (min_diag <= 0.0) {
    ...
    // copy
    mju_copy(hessian_band_factor, hessian_band, ntotal * nband);
