...
TEST(ForceCost, ParticleDamped) {
  ...
  double a1_fd[1] = {
      (sim.qpos.Get(2)[0] - 2.0 * sim.qpos.Get(1)[0] + sim.qpos.Get(0)[0]) /
      (model->opt.timestep * model->opt.timestep)};
  printf("a1 (fd) = %f\n", a1_fd[0]);

  mju_copy(data->qpos, sim.qpos.Get(1), model->nq);
  mju_copy(data->qvel, sim.qvel.Get(1), model->nv);
  mju_copy(data->qacc, a1_fd, model->nv);
  ...
  // weights
  estimator.noise_process[0] = 1.0e-1;
  estimator.noise_process[1] = 2.0e-1;
