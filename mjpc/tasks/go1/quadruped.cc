...
std::string Go1::XmlPath() const { return GetModelPath("go1/task.xml"); }
std::string Go1::Name() const { return "Go1"; }

void Go1::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  for (Go1Foot foot : kFootAll) {
    if (is_biped) {
      ...
      bool back_hand = handstand && (foot == kFootRL || foot == kFootRR);
...
//  ============  transition  ============
void Go1::TransitionLocked(mjModel* model, mjData* data) {
  // ---------- handle mjData reset ----------
  ...
  if (gait_selection != residual_.current_gait_) {
    ...
    ResidualFn::Go1Gait gait = residual_.GetGait();
  ...
  // save mode
  residual_.current_mode_ = static_cast<ResidualFn::Go1Mode>(mode);
...
void Go1::ModifyScene(const mjModel* model, const mjData* data,
  ...
  for (ResidualFn::Go1Foot foot : ResidualFn::kFootAll)
  ...
  for (ResidualFn::Go1Foot foot : ResidualFn::kFootAll) {
    ...
    if (residual_.current_mode_ == ResidualFn::kModeBiped) {
      ...
      bool back_hand = handstand && (foot == ResidualFn::kFootRL ||
                                     foot == ResidualFn::kFootRR);
...
void Go1::ResetLocked(const mjModel* model) {
  // ----------  task identifiers  ----------
...
void Go1::ResidualFn::AverageFootPos(double avg_foot_pos[3],
  ...
  if (current_mode_ == kModeBiped) {
    ...
    if (handstand) {
      ...
      mju_add3(avg_foot_pos, foot_pos[kFootRL], foot_pos[kFootRR]);
    }
...
double Go1::ResidualFn::GetPhase(double time) const {
  return phase_start_ + (time - phase_start_time_) * phase_velocity_;
}

// horizontal Walk trajectory
void Go1::ResidualFn::Walk(double pos[2], double time) const {
...
// get gait
Go1::ResidualFn::Go1Gait Go1::ResidualFn::GetGait() const {
  if (current_mode_ == kModeBiped) return kGaitTrot;
  return static_cast<Go1Gait>(ReinterpretAsInt(current_gait_));
}

// return normalized target step height
double Go1::ResidualFn::StepHeight(double time, double footphase,
...
void Go1::ResidualFn::FootStep(double step[kNumFoot], double time,
                               Go1Gait gait) const {
...
// height during flip
double Go1::ResidualFn::FlipHeight(double time) const {
...
void Go1::ResidualFn::FlipQuat(double quat[4], double time) const {
  double angle = 0;
  if (time >= jump_time_ + flight_time_ + land_time_) {
    angle = 2 * mjPI;
