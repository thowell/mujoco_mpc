...
class GenerateProtoGrpcCommand(setuptools.Command):
  """Specialized setup command to handle unscented proto compilation.

  Generates the `unscented_pb2{_grpc}.py` files from `unscented_proto`. Assumes that
  ...
  def run(self):
    """Generate `unscented.proto` into `unscented_pb2{_grpc}.py`.
    ...
    for the generated `unscented_pb2_grpc.py` file. The final argument of the
    `protoc` call has to be "mujoco_mpc/unscented.proto" in order for the import to
    become `from mujoco_mpc import [unscented_pb2_proto_import]` instead of just
    `import [unscented_pb2_proto_import]`. The latter would fail because the name is
    ...
    protoc_command_parts = [
        ...
        str(unscented_proto_destination_path),
    ]
...
class CopyKALMANServerBinaryCommand(setuptools.Command):
  ...
  description = "Copy `unscented_server` next to `unscented.py`."
  user_options = []
  ...
  def run(self):
    self._copy_binary("unscented_server")
    # self._copy_binary("ui_unscented_server")
...
class CopyTaskAssetsCommand(setuptools.Command):
  """Copies `unscented_server` and `ui_unscented_server` next to `unscented.py`.
...
class BuildPyCommand(build_py.build_py):
  """Specialized Python builder to handle unscented service dependencies.

  During build, this will generate the `unscented_pb2{_grpc}.py` files and copy
  `unscented_server` binary next to `unscented.py`.
  """
...
class BuildCMakeExtension(build_ext.build_ext):
  ...
  def run(self):
    self._configure_and_build_unscented_server()
    self.run_command("copy_unscented_server_binary")

  def _configure_and_build_unscented_server(self):
    """Check for CMake."""
    cmake_command = "cmake"
    build_cfg = "Debug"
    ...
    print("Building `unscented_server` and `ui_unscented_server` with CMake")
    subprocess.check_call(
        [
            cmake_command,
            "--build",
...
setuptools.setup(
    ...
    ext_modules=[CMakeExtension("unscented_server")],
    cmdclass={
    ...
    package_data={
        "": [
            "mjpc/unscented_server",
            # "mjpc/ui_unscented_server",
        ],
    },
)
